 ...
 *
 * Persistence uses Postgres COPY (not parameterized batch INSERTs), run
 * sequentially table by table (not concurrently) -- only for
 * resolved_entities, proposals, exceptions, and audit_events, since
 ...
 * scale writing them anyway is ~300k rows nothing queries. Running the four
 * remaining COPY streams concurrently (one connection each) was tried and
 * measured slower in practice than running them one at a time immediately
 * after the CPU-heavy pipeline compute phase, even though each individual
 * COPY is fast in isolation (a standalone benchmark moved the full ~40MB
 * dataset in ~1s) -- sequential is simply the reliable configuration.
 */
...
export async function runPipelineForJob(jobId: string, sourceFiles: SourceFile[]): Promise<PipelineRunResult> {
  ...
  try {
    ...
    await copyInsert("audit_events", ["id", "job_id", "type", "actor", "payload", "at"], final.audit, (a) => [
      a.id,
      jobId,
      a.type,
      a.actor,
