...
describe("demo-store", () => {
  ...
  it("syncFromPath maps each URL to the right phase", () => {
    ...
    expect(useDemoStore.getState().phase).toBe("connect");
    syncFromPath("/migrate/processing");
    expect(useDemoStore.getState().phase).toBe("processing");
    syncFromPath("/workspace");
    expect(useDemoStore.getState().phase).toBe("workspace");
  });
  ...
  it("syncFromPath falls back to landing for unknown paths", () => {
    useDemoStore.getState().syncFromPath("/workspace");
  ...
  it("PHASE_TO_PATH has all 4 phases", () => {
    ...
    expect(phases).toContain("workspace");
  });
